...
object Sudoku {
  ...
  /** Deletes the value v from all sets of potential digits in
    * the same collection.
    */
  ...
  /** Checks if the given value can be found in other fields in
    * the same row.
    */
  ...
  /**
    * Checks if the given value can be found in the other fields in
    * the same row, column and submatrix.
    */
  ...
  /**
    ...
    * and submatrix.
    */
  def transformation1(p: Puzzle[Set[Int]],
                      r: Int,
                      c: Int,
                      s: Set[Int]): Option[Puzzle[Set[Int]]] =
    for {
      nr <- nthRow(p, r)
      dr = deleteFrom(nr, s)
      pr <- patchRow(p, r, dr)
  ...
  /** Finds the value for which the second type of transformation
    * will be carried out.
    */
  ...
  /**
    * Called when a field which can hold a potential value which is
    * not found in any other set in the same row, column or submatrix.
    * That value replaces the whole set.
    */
  def transformation2(p: Puzzle[Set[Int]],
                      r: Int,
                      c: Int,
                      v: Int): Option[Puzzle[Set[Int]]] =
    for {
      row <- nthRow(p, r)
  ...
  private def chooseTransformation(p: Puzzle[Set[Int]],
                                   r: Int,
                                   c: Int,
                                   s: Set[Int]): Option[Puzzle[Set[Int]]] =
    if (isSingle(s)) transformation1(p, r, c, s)
    else
      findValue(p, r, c, s).fold(Option(p)) { v =>
